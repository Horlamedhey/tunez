...
if ($_POST['email'] && $_POST['password']) {
  ...
  if ($entries < 10000) {
    ...
    if (!$checkEmail) {
      if ($referrerCode !== '') {
        if (DB::queryFirstRow("SELECT ownReferralCode FROM usersTable WHERE
          ownReferralCode=%s", $referrerCode)) {
          if (DB::insert('usersTable', array(
            'email' => $email,
          ...
          if ($ref = DB::queryFirstRow("SELECT referrals FROM usersTable WHERE ownReferralCode=%s", $referrerCode)) {
        ...
        if (DB::insert('usersTable', array(
          ...
          // $ref = DB::queryFirstRow("SELECT referrals FROM usersTable WHERE ownReferralCode=%s", '2mfCp6Hd');
          // DB::update('usersTable', array(
          //   'referrals' => $ref['referrals'] + 1
          // ), 'ownReferralCode=%s', '2mfCp6Hd');
          echo ('Registered Successfully.');
          // $user = DB::queryFirstRow("SELECT * FROM usersTable WHERE email=%s AND password=%s", $email, $userPassword);
          // Session::set($user);
        } else {
